...
# shown in digest-hybrid. This proves the render code is correctly selecting the
# digest-hybrid branch even when results exist. The truly-empty-index case is
...
# digest.json present → digestRequested=true → verdict resolves digest-hybrid.
...
# Its absence proves digest-hybrid render branch was selected.
scn_assert_pane_not_contains "may still be building" \
    "S14: legacy empty-state message NOT shown in digest-hybrid (proves correct render branch)"
