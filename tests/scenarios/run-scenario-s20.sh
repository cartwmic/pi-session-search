#!/usr/bin/env bash
# Scenario S20 — Index v3 → v4 hard-reset on load
#
...
#   3. Extension calls index.load() which detects v3 ≠ v4, resets, notifies
#   4. Assert pane shows incompatibility notify
#   5. Assert file is now version 4
...
# Primary assertion: the index file IS rewritten to version 4. This is the
# canonical proof that the migration code (`migrateIndexFileIfStale`) executed
# regardless of the active mode (fts-raw / hybrid-raw / digest-mode) — the
...
scn_assert_file_contains \
    ...
    '"version":[ ]*4' \
    "S20: index file rewritten to version 4 (migration ran)"
...
if grep -qE "[Ii]ncompatible|reset to v4|digest:backfill" "$PANE_LOG"; then
