...
describe("registration (Phase B) — module-load invariants", () => {
  ...
  it("3. search/digest tools and commands installed at module-load", async () => {
    ...
    assert.ok(cmds.includes("session:summarizer"), "session:summarizer")
    assert.ok(cmds.includes("session:update"), "session:update")
    assert.ok(cmds.includes("session:digest"), "session:digest")
    assert.ok(cmds.includes("session:rewrite"), "session:rewrite")
    assert.ok(cmds.includes("session:backfill"), "session:backfill")
    assert.ok(cmds.includes("session:cost"), "session:cost")
    assert.ok(cmds.includes("session:embedder"), "session:embedder")
